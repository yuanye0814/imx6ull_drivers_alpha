...
static void do_ram_disk_make_request(struct request_queue *q, struct bio *bio)
{
    struct ram_disk_dev *dev = q->queuedata;
    struct bio_vec bv;
    ...
    bio_for_each_segment(bv, bio, iter) {
        void *buffer = kmap(bv.bv_page) + bv.bv_offset;
        unsigned long len = bv.bv_len;

        if ((offset + len) > RAM_DISK_SIZE) {
            printk(NAME " BIO out of bounds: start_sector=%llu, len=%lu\n",
                   (unsigned long long)start_sector, len);
            err = -EIO;
            break;
        }
...
static int __init ram_disk_driver_init(void)
{
    ...
    // allocate queue
    ram_disk.request_queue = blk_alloc_queue(GFP_KERNEL);
    if (!ram_disk.request_queue) {
        printk(NAME " failed to allocate blk queue\n");
        ret = -ENOMEM;
        goto error_init_request_queue;
    }
    ram_disk.request_queue->queuedata = &ram_disk;

    // blk queue make request
    blk_queue_make_request(ram_disk.request_queue, do_ram_disk_make_request);
